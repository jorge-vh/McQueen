...
class puzzlebot_cinematic_model:
   def __init__(self):
   ...
   #Crear el set de velocidades angulares y lineales
   def setVel(self,msg):
      self.vel_angular = msg.angular.z
      self.vel_linear = msg.linear.x

   #Obtener la posicion del puzzlebot en x,y,z
   ...
   #Actualizar la posicion de el Puzzlebot
   ...
   def broadcastTransform(self, orientation):
        ...
        #definicion de mensaje de odometria
        odometry = Odometry()
   ...
   #funcion para obtener covarianza en forma elipsoidal
   def getCovariance(self):
      ...
      #EL kl y kr (en este caso 1) es el valor que se tiene que tunnear para que tener resultados diferentes
      #checar la diferente variacion, K es una constante que nos va a servir como una variacion 
      #de la simulacion hacia lo que en realidad es
      Edk = np.matrix([[1*abs((self.vel_linear + self.vel_angular)/self.r), 0],
                      [0, 1*abs((self.vel_linear - self.vel_angular)/self.r)]])
      
      #En base a presentacion funcion para obtener QK
      Qk = wk*Edk*wkt
...
if __name__ == '__main__':
   rospy.init_node('Locatisation')
   puzzlebot = puzzlebot_cinematic_model()
   rate = rospy.Rate(150)
