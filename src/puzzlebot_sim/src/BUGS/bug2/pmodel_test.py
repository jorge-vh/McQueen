...
class puzzlebot_cinematic:
   def __init__(self):
      ...
      self.freq = 100
      # Frames
      self.header = 'r/base_link'
      self.child = 'r/chassis'
   ...
   def getPuzzleVel(self):
      ...
      self.angle = self.vel_angular / self.freq
      self.getPuzzlePos()
   
   # Asignar la posicion al topico de pose_sim
   ...
   # Enviar la transformada usando un broadcaster
   ...
   # Calcular la velocidad en cada rueda
   def getWheelVel(self):
      self.wl.publish((self.vel_linear*2 - self.vel_angular*self.l)/(2*self.r))
      self.wr.publish((self.vel_linear*2 + self.vel_angular*self.l)/(2*self.r))      
