...
received_abierto_message = False
...
# Callback function for gripperSTM messages
def gripper_stm_callback(msg):
    global received_cerrado_message, received_abierto_message

    if msg.data == "cerrado":
        received_cerrado_message = True
    elif msg.data == "abierto":
        received_abierto_message = True
...
def control_loop(target_aruco_id):
    ...
    # Subscribe to gripperSTM topic
    rospy.Subscriber('gripperSTM', String, gripper_stm_callback)

    # Publish velocity commands
    cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10)

    # Publish "gripperJetson" messages
    gripper_jetson_pub = rospy.Publisher("gripperJetson", String, queue_size=10)
    ...
    while not rospy.is_shutdown():
        if current_marker is not None and current_marker.id == target_aruco_id:
            ...
            if marker_area > 0.08 * image_width * image_height and (current_marker.id == 0 or current_marker.id == 2 or current_marker.id == 1):
                ...
                # Reset received_cerrado_message flag
                received_cerrado_message = False
            ...
            if marker_area > 0.17 * image_width * image_height and (current_marker.id == 10 or current_marker.id == 11 or current_marker.id == 12):
                # Publish "soltar" message
                gripper_jetson_pub.publish("soltar")

                # Wait until "abierto" message is received
                while not received_abierto_message:
                    rate.sleep()

                # Reset received_abierto_message flag
                received_abierto_message = False
